 ...
 * All Rights Reserved.
 * net.crytec.cosmetic.particle.effects.EarthFairy can not be copied and/or distributed without the express
...
public class EarthFairy extends IParticleEffect {

  private static final BlockData data_dirt = Bukkit.createBlockData(Material.DIRT);
  private static final BlockData data_water = Bukkit.createBlockData(Material.WATER);
  ...
  public EarthFairy(final Player player) {
    super(player);
  ...
  public void onUpdate(final boolean isMoving) {
    ...
    Particle.VILLAGER_HAPPY.builder().location(currentLocation).count(1).extra(0).offset(0, 0, 0).receivers(64).spawn();
    Particle.BLOCK_CRACK.builder().location(currentLocation).count(1).offset(0.05, 0.05, 0.05).extra(1).data(data_dirt).receivers(64).spawn();
    Particle.BLOCK_CRACK.builder().location(currentLocation).count(1).offset(0.05, 0.05, 0.05).extra(1).data(data_water).receivers(64).spawn();
